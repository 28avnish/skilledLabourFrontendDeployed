...
function Bookingscreen() { 
  ...
  useEffect(() => {
    ...
    try {
      setloading(true);
      const data = (await axios.post("https://skilledlabour.onrender.com/api/services/getservicebyid", {serviceid: serviceid })).data;
      setservice(data);
      setloading(false);
    } catch (error) {
      setloading(false);
  ...
  async function onToken(token){
     ...
     const bookingDetails = {
      ...
      try {
        setloading(true)
        const result = await axios.post('https://skilledlabour.onrender.com/api/bookings/bookservice', bookingDetails)
        setloading(false)     
